...
pub const SFD = struct {
    ...
    pub fn updateFusedFisher(self: *SFD, gradients: *const Tensor, params: *Tensor, lr: f32) !void {
        if (!self.initialized) return error.NotInitialized;
        if (!std.math.isFinite(lr) or lr < 0.0) return error.InvalidLearningRate;
        if (gradients.data.len != params.data.len) return error.ShapeMismatch;
        if (gradients.data.len != self.param_size) return error.ShapeMismatch;

        self.step_count +|= 1;

        const g_data = gradients.data;
        const w_data = params.data;
        const m_data = self.momentum_buffer.data;
        const f_data = self.fisher_diag.data;

        const beta = std.math.clamp(self.beta1, @as(f32, 0.0), @as(f32, 0.999999));
        const gamma = std.math.clamp(self.beta2, @as(f32, 0.0), @as(f32, 0.999999));
        const eps = self.eps;
        const f16_bound: f32 = 65504.0;

        const warmup_steps_f = @as(f32, @floatFromInt(self.warmup_steps));
        const step_f = @as(f32, @floatFromInt(self.step_count));
        const warmup_factor: f32 = if (self.step_count <= self.warmup_steps and self.warmup_steps > 0) step_f / warmup_steps_f else 1.0;
        const eff_lr = lr * warmup_factor;

        const VLEN: usize = 8;
        const vbeta: @Vector(VLEN, f32) = @splat(beta);
        const vgamma: @Vector(VLEN, f32) = @splat(gamma);
        const vone_minus_gamma: @Vector(VLEN, f32) = @splat(1.0 - gamma);
        const veps: @Vector(VLEN, f32) = @splat(eps);
        const veff_lr: @Vector(VLEN, f32) = @splat(eff_lr);
        const vbound: @Vector(VLEN, f32) = @splat(f16_bound);
        const vneg_bound: @Vector(VLEN, f32) = @splat(-f16_bound);
        const vfisher_max: @Vector(VLEN, f32) = @splat(self.fisher_max);
        const vzero: @Vector(VLEN, f32) = @splat(0.0);
        const vone: @Vector(VLEN, f32) = @splat(1.0);
        const vinf: @Vector(VLEN, f32) = @splat(std.math.inf(f32));

        var i: usize = 0;
        while (i + VLEN <= self.param_size) : (i += VLEN) {
            const g: @Vector(VLEN, f32) = g_data[i..][0..VLEN].*;
            const g_nonan = @select(f32, g == g, g, vzero);
            const g_safe = @select(f32, @abs(g_nonan) != vinf, g_nonan, vzero);
            const m_old: @Vector(VLEN, f32) = m_data[i..][0..VLEN].*;
            const m_new = vbeta * m_old + g_safe;
            m_data[i..][0..VLEN].* = m_new;
            const f_old: @Vector(VLEN, f32) = f_data[i..][0..VLEN].*;
            var f_new = vgamma * f_old + vone_minus_gamma * g_safe * g_safe;
            f_new = @min(f_new, vfisher_max);
            f_new = @select(f32, f_new == f_new, f_new, vone);
            f_data[i..][0..VLEN].* = f_new;
            const w_old: @Vector(VLEN, f32) = w_data[i..][0..VLEN].*;
            const update_val = veff_lr * (m_new / (@sqrt(f_new) + veps));
            const w_pre = w_old - update_val;
            const w_clamped = @min(@max(w_pre, vneg_bound), vbound);
            w_data[i..][0..VLEN].* = @select(f32, w_pre == w_pre, w_clamped, w_old);
        }
        while (i < self.param_size) : (i += 1) {
            const g = g_data[i];
            if (!std.math.isFinite(g)) continue;
            m_data[i] = beta * m_data[i] + g;
            var f_new = gamma * f_data[i] + (1.0 - gamma) * g * g;
            f_new = @min(f_new, self.fisher_max);
            if (!std.math.isFinite(f_new)) f_new = 1.0;
            f_data[i] = f_new;
            const update_val = eff_lr * m_data[i] / (@sqrt(f_new) + eps);
            const w_new = std.math.clamp(w_data[i] - update_val, -f16_bound, f16_bound);
            if (std.math.isFinite(w_new)) w_data[i] = w_new;
        }
    }
    ...
    pub fn updateFusedFisherMulti(self: *SFD, gradients: []const Tensor, params: []*Tensor, lr: f32) !void {
        if (!self.initialized) return error.NotInitialized;
        var total: usize = 0;
        for (params) |p| total += p.data.len;
        if (total != self.param_size) return error.ShapeMismatch;
        var offset: usize = 0;
        for (params, 0..) |p, idx| {
            const n = p.data.len;
            if (gradients[idx].data.len != n) return error.ShapeMismatch;
            const beta = std.math.clamp(self.beta1, @as(f32, 0.0), @as(f32, 0.999999));
            const gamma = std.math.clamp(self.beta2, @as(f32, 0.0), @as(f32, 0.999999));
            const warmup_steps_f = @as(f32, @floatFromInt(self.warmup_steps));
            const step_f = @as(f32, @floatFromInt(self.step_count + 1));
            const warmup_factor: f32 = if (self.step_count + 1 <= self.warmup_steps and self.warmup_steps > 0) step_f / warmup_steps_f else 1.0;
            const eff_lr = lr * warmup_factor;
            var i: usize = 0;
            while (i < n) : (i += 1) {
                const g = gradients[idx].data[i];
                if (!std.math.isFinite(g)) continue;
                const mi = offset + i;
                self.momentum_buffer.data[mi] = beta * self.momentum_buffer.data[mi] + g;
                var f_new = gamma * self.fisher_diag.data[mi] + (1.0 - gamma) * g * g;
                f_new = @min(f_new, self.fisher_max);
                if (!std.math.isFinite(f_new)) f_new = 1.0;
                self.fisher_diag.data[mi] = f_new;
                const update_val = eff_lr * self.momentum_buffer.data[mi] / (@sqrt(f_new) + self.eps);
                const w_new = std.math.clamp(p.data[i] - update_val, @as(f32, -65504.0), @as(f32, 65504.0));
                if (std.math.isFinite(w_new)) p.data[i] = w_new;
            }
            offset += n;
        }
        self.step_count +|= 1;
    ...
    pub fn writeBackFP16(params: *const Tensor, dst: []f16) !void {
        ...
        var i: usize = 0;
        const n = params.data.len;
        while (i < n) : (i += 1) {
            const v = params.data[i];
            dst[i] = @floatCast(std.math.clamp(v, @as(f32, -65504.0), @as(f32, 65504.0)));
        }
    }
...
test "SFD fused Fisher update matches persistent momentum Fisher reference" {
    ...
    while (i < 16) : (i += 1) {
        ...
        const m = opt.beta1 * 0.0 + g;
        const f = @min(opt.beta2 * 1.0 + (1.0 - opt.beta2) * g * g, opt.fisher_max);
        const expected = initial_params[i] - lr * m / (@sqrt(f) + opt.eps);
...
test "SFD fused multi-layer update applies per-tensor slices" {
    ...
    const m = opt.beta1 * 0.0 + 0.1;
    const f = @min(opt.beta2 * 1.0 + (1.0 - opt.beta2) * 0.01, opt.fisher_max);
    const expected1 = 0.05 - 0.01 * m / (@sqrt(f) + opt.eps);
